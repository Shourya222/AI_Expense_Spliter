...
export const getUserBalances = query({
  handler: async (ctx) => {
    ...
    for (const e of expenses) {
      ...
      if (isPayer) {
        for (const s of e.splits) {
          //Skip user's own slplit or already paid splits
    ...
    // Get setllements that directly involve the current user
    ...
    /* build lists for UI */
    const youOweList = []; //List of people user owes the money to
